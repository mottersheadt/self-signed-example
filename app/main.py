...
CALM_DOMAIN = os.environ.get('CALM_DOMAIN')
...
def checkout():
    ...
    try:
        ...
        calm_enroll(context, fin_instr_data, access_token)
...
def calm_enroll(context, fin_instr_data, access_token):
    update_stage(context, "calm-enroll", "enrolling in CALM")
    trace(context, "====== ENROLLING CARD IN CALM ======")
    payload = {
        "name": "Django", # TODO: how to get this with Checkout?
        "number": fin_instr_data["card"]["number"],
        "exp_month": fin_instr_data["card"]["exp_month"],
        "exp_year": fin_instr_data["card"]["exp_year"],
        "capabilities": [
            "ACCOUNT_UPDATER"
        ]
    }
    calm_enroll_url = 'https://' + CALM_DOMAIN + '/cards'
    proxy_url = 'https://' + CUSTOMER_VAULT_ACCESS_CREDS_USERNAME + ':' + CUSTOMER_VAULT_ACCESS_CREDS_SECRET + '@' + CUSTOMER_VAULT_ID + '.sandbox.verygoodproxy.com:8443'
    sanitized_proxy_url = 'https://' + CUSTOMER_VAULT_ACCESS_CREDS_USERNAME + ':***ACCESS_CREDENTIAL_PASSWORD***@' + CUSTOMER_VAULT_ID + '.sandbox.verygoodproxy.com:8443'

    trace(context, "Enrolling card in CALM by sending POST request to:")
    trace(context, "- " + calm_enroll_url)
    trace(context, "Proxying enroll request through outbound route URL:")
    trace(context, "- " + sanitized_proxy_url)
    ...
    formatted_json = json.dumps(enrollment.json(), indent=4)
    trace(context, "Enrollemnt Response:" + formatted_json)
